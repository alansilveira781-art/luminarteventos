...
export function NotificacoesPanel({ onNavigate }: { onNavigate?: () => void }) {
  const { user } = useAuth();
  const qc = useQueryClient();
  const [filtro, setFiltro] = useState<"todas" | "nao_lidas" | "pendentes" | "concluidas">("pendentes");
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="divide-y divide-border max-h-[55vh] overflow-y-auto">
        {filtered.length === 0 && (
        ...
        {filtered.map((n) => (
          <div key={n.id} className={`p-3 flex items-start gap-3 ${!n.lida ? "bg-accent/20" : ""}`}>
            ...
            <div className="flex items-center gap-1">
              {n.link && (
                <Link to={n.link as any} onClick={() => { marcarLida.mutate(n.id); onNavigate?.(); }}>
